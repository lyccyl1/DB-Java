        ...
        if (flag == 1) {
            while (arr.size() == 0) {
                ...
                if (f == 1) {
                    ...
                    System.out.println("请输入请输入需要检索的关键字个数(精准匹配)输入-1退出:(proteins;sequences;maxsize=2)");
                    ...
                    for (int i = 0; i < n; i++) {
                        System.out.println("请输入需要检索的序列(精准匹配)：（proteins:1;sequences:2）");
                        key = scan_data.scanint() - 1;
                        System.out.println("请输入序列:" + a[key] + "的检索关键字(精准匹配)");
                        name = scan_data.scan();
                        arr = operation_out_data.search_in_data(arr, key, name);
                    }
                } else if (f == 2) {
                    int key;
                    String name;
                    ...
                    String[] a = choice.split(",");
                    System.out.println("请输入需要检索的关键字个数(精准匹配)输入-1退出:(index;proteins;sequences;annotations;maxsize=4)");
                    int n = scan_data.scanint();
                    if (n == -1) {
                        System.out.println("Program exit");
                        br=0;
                    ...
                    while (n > 4 || n < -1 || n == 0) {
                        System.out.println("输入不符合要求！再来一次");
                        System.out.println("请输入需要检索的关键字个数(精准匹配)输入-1退出:(index:1;proteins:2;sequences:3;annotations:4;maxsize=4)");
                        n = scan_data.scanint();
                    }
                    if (n == -1) {
                        System.out.println("Program exit");
                    ...
                    for (int i = 0; i < n; i++) {
                        System.out.println("请输入需要检索的序列(精准匹配):(index:1;proteins:2;sequences:3;annotations:4)");
                        key = scan_data.scanint() - 1;
                        System.out.println("请输入序列:" + a[key] + "的检索关键字(精准匹配)");
                        name = scan_data.scan();
                        arr = operation_out_data.search_in_data(arr, key, name);
                    ...
                    String[] a = choice.split(",");
                    System.out.println("请输入请输入需要检索的关键字个数(精准匹配)输入-1退出:(index;proteins;accessions;sequences;annotations;interpros;orgs;maxsize=7)");
                    int n = scan_data.scanint();
                    if (n == -1) {
                        System.out.println("Program exit");
                        br=0;
                    ...
                    while (n > 7 || n == 0 || n < -1) {
                        System.out.println("输入不符合要求！再来一次");
                        System.out.println("请输入需要检索的关键字个数(精准匹配)输入-1退出:(index=1;proteins=2;accessions=3;sequences=4;annotations=5;interpros=6;orgs=7;maxsize=7)");
                        n = scan_data.scanint();
                    }
                    if (n == -1) {
                        System.out.println("Program exit");
                    ...
                    for (int i = 0; i < n; i++) {
                        System.out.println("请输入需要检索的序列(精准匹配):(index=1;proteins=2;accessions=3;sequences=4;annotations=5;interpros=6;orgs=7)");
                        key = scan_data.scanint() - 1;
                        System.out.println("请输入序列:" + a[key] + "的检索关键字(精准匹配)");
                        name = scan_data.scan();
                        arr = operation_out_data.search_in_data(arr, key, name);
            ...
            if (br==1){
                System.out.println("输入关键字精准匹配到"+arr.size()+"个结果,每1000个为一页，请输入需要查询的页码：");
                int n=0;
                int x=0;
                if (arr.size()%1000==0){
                    x = arr.size()/1000;
                ...
                while((n=scan_data.scanint())!=-1){
                    ...
                    if(h==1){
                        ...
                        while(!myname.substring(myname.length()-4).equals(".tsv")&&!myname.substring(myname.length()-4).equals(".csv")) {
                            System.out.println("你取的名字有问题哦 请以.csv or .tsv 结尾");
